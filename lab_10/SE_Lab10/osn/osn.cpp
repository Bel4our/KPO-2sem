...
int main()
{
	...
	std::cout << std::endl << " Лямбда, захват переменных" << std::endl; // лямбда-выражение, захват переменных
	...
	std::cout << std::endl << " Лямбда с параметрами" << std::endl; // Вывод значений кратных 3. Значения будем передавать как параметр у обычной функции
	for (auto i: v) [] (auto i) 
		{
		if (i%3== 0)
			std::cout << i << " "; 
